...
def register(l):
    _user = getattr(l, 'p_user', getattr(l.parent, 'p_user', Faker().simple_profile()))
    _password = getattr(l, 'p_password', getattr(l.parent, 'p_password', Faker().password()))
    details = {
...
def login(l):
    ...
    p_user = [p_user for p_user in users if p_user.get('username') == _user.get('username')]
    if not p_user:
        register(l)

    base64string = base64.encodebytes(('%s:%s' % (_user.get('username'), _password)).encode()).decode().strip()
...
class WebTasks(TaskSet):
    ...
    p_user = Faker().simple_profile()
    p_password = Faker().password()
...
class UsersTasks(TaskSet):
    ...
    def post_customers(self):
        ...
        p_password = Faker().password()
        data = {
            'firstName': p_user.get('name').split(' ')[0],
            'lastName': p_user.get('name').split(' ')[1],
            'email': p_user.get('mail'),
    ...
    def post_register(self):
        self.p_user = Faker().simple_profile()
        self.p_password = Faker().password()
